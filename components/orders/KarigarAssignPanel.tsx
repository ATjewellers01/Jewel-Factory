 ...
 */
/**
 * Just the dropdown (codes + "More…" + "+ Add new Karigar Code…" + inline
 * remove), no "Assign items" trigger — for callers that already have their
 * own submit action elsewhere (e.g. AssignKarigarModal's own footer button).
 * KarigarPicker below wraps this with the trigger for the outer-dropdown
 * flow (RetailerCustomRequestDetail, which has no linked items to check off).
 */
export function KarigarSelect({
  ...
  async function removeCode(karigar: Karigar, e: React.MouseEvent) {
    ...
    try {
      ...
      if (karigarId === karigar.id) onPick(null);
    } catch (e2) {
