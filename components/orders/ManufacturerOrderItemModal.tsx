...
/**
 * Manufacturer-facing product detail popup for an order line item — shows
 * everything the catalog detail would, PLUS karigarCode (safe here since this
 * is manufacturer-only, unlike the retailer/store-manager product modal which
 * never receives karigarCode from the API in the first place).
 */
export function ManufacturerOrderItemModal({ product, onClose }: { product: OrderItemProduct; onClose: () => void }) {
  useEffect(() => {
