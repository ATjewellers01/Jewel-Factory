...
- **[x] Customised-order spec fields** (migration `20260801000000_custom_design_spec_fields`) — `sub_category`, `order_ref`, `delivery_date`, `quantity`, `meena`, `length`, `size`, `broadness`, `screw`, `sample_weight_grams` added to **both** `custom_design_requests` and `custom_design_orders`, all nullable. `forwardCustomRequest` copies the whole spec through to the manufacturer (it carries no PII). New shared `components/orders/CustomSpecList.tsx` renders only the filled fields, used by the Retailer User (My Orders), Retailer Admin (Customised Orders) and manufacturer.
  - `orderRef` is the **shop's own** order number — NOT `CustomDesignOrder.orderNumber` (the system's `CD-YYYYMMDD-XXXX`). Don't merge them.
