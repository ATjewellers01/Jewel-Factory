...
- **Customer PII is NOT stored and never reaches the manufacturer.** Kiosk/custom orders carry only products + qty + an editable `requirementNote`. Customer name/phone are nullable (store manager keeps them outside the system). **Manufacturer sees: the retailer's business name (e.g. "ABC"), requirement note/remarks, retailer HO ship-to address, product/spec detail — but NOT the retailer's city or branch name.** (2026-08-05: store identity was briefly removed entirely from manufacturer order views for privacy, then partially reinstated same day — client wanted the business name back to identify who placed each order, just not city/branch. `getB2bOrdersByManufacturer`/`getB2bOrderForManufacturer` join `store: { select: { name: true } }` only; `sanitizeKiosk`'s denylist drops `storeCitySnapshot`/`branchNameSnapshot` but lets `storeNameSnapshot` through; `listCustomOrdersByManufacturer` selects `storeNameSnapshot`; the dashboard's recent-orders widget carries `storeName`. Any NEW store-identity field (city, branch, address components beyond ship-to) must stay off these same points — only the business name is allowed through.)
...
## Migrations (22, all Prisma-managed, idempotent)
`0001 jewel_factory` · `kiosk_pin` · `b2b_item_image` · `branch_hierarchy` (branches + branch_managers + branch_id/requirement_note on orders + nullable PII) · `order_messages` (order_messages table + OrderKind/MessageSender enums + completed_at on kiosk/b2b/custom) · `add_analytics_indexes` · `custom_design_weight_range` · `pgvector` (adds the `vector(512)` embedding column on `manufacturer_product_embeddings`, used by pgvector search — see External services) · `extra_branch_allowance` · **`product_karigar_pieces_nullable_name`** (2026-07-30: `ManufacturerProduct.name` DROP NOT NULL, adds `pieces` + `karigar_code`) · **`favorite_products`** (2026-07-30: new table, see Core rules) · **`retailer_badges`** (2026-07-30: `manufacturers.retailer_badge_labels` + `stores.badge_label`) · **`retailer_delete_cascade`** (2026-07-30: `b2b_orders`/`kiosk_orders`/`custom_design_orders` FK to stores switched from `ON DELETE RESTRICT` to `CASCADE` — see Gotchas) · **`favorite_kind_kiosk_restock`** (2026-07-30: adds `FavoriteKind` enum + `kind` column on `favorite_products`, unique/index widened to include it — see Status) · **`store_email_optional`** (2026-07-31: `stores.email` DROP NOT NULL + index on `owner_phone` — mobile-only purchase manager signup, see Gotchas) · **`custom_design_spec_fields`** (2026-08-01: sub-category + the counter spec — order ref, delivery date, quantity, meena, length, size, broadness, screw, sample weight — on BOTH `custom_design_requests` and `custom_design_orders`, all nullable) · **`product_size`** (2026-08-02: optional `size` on `manufacturer_products` — bangle sizing, form-gated to the Bangles category) · **`custom_design_quantity_text`** (2026-08-03: `quantity` on custom design requests/orders widened `Int?` → `String?` — free text like "2 pcs") · **`kiosk_sales_person`** (2026-08-03: `sales_code` + `sales_person_name` on `kiosk_orders`, captured at Store Manager "Add to Cart") · **`order_status_rework`** (2026-08-03: `OrderStatus`/`CustomOrderStatus` enums replaced — see Status) · **`order_item_status`** (2026-08-03: `status OrderStatus` added to `kiosk_order_items`/`b2b_order_items` — per-line-item status) · **`manufacturer_order_seq`** (2026-08-04: `manufacturers.next_catalog_order_seq`/`next_custom_order_seq` — backs JFA-/JFC- order numbers, see Status). `pnpm db:deploy` applies all. `migrate:categories`/`migrate:branches` = one-off upgrades for an EXISTING DB only.

## Status

**Latest session (2026-08-05) — customised-order merge/privacy/badges, registration + login polish, Order History simplification, responsive nav labels:**
- **[x] Customised Design removed from Retailer User (Store Manager) portal entirely** — `/store-manager/custom-design` page deleted, nav/footer links removed, My Orders' Customised tab removed. The Store Manager's `BranchOrderList` no longer shows the manufacturer's granular per-item status or "Manufacturer Status" block — only a simple Pending/Approved/Completed badge (`statusOf`/`bucketOf` in `lib/order-filters.ts`), since the manufacturer's real production status is HO-only info now.
- **[x] Sales Code + Sales Person Name removed entirely** (migration `20260804030000_remove_sales_person_multi_image` drops the columns from `kiosk_orders`, `custom_design_requests`, `custom_design_orders`) — schema, Zod bodies, UI (`CatalogOrderPanel`, order lists), and `use-store-manager-cart.ts` all cleaned up.
- **[x] Order numbering unified** — Customised orders now draw from the same `nextCatalogOrderSeq` counter as Kiosk/Catalog (`JFA-####`); the separate `nextCustomOrderSeq`/`JFC-####` path (`nextCustomOrderNumber`) is dead code, left in place unused rather than removed.
- **[x] Retailer Admin's Order History (`/store/b2b-orders`, renamed from "Catalogue Orders")** now merges Restock/Kiosk/Customised orders in one list (was already merging Restock+Kiosk). Customised orders auto-forward on placement (no approval step, matching the existing self-placed-order precedent) — the old separate approve/reject-driven Customised Orders list page is gone; `/store/custom-designs` now just redirects here. New multi-image support for customised orders (`referenceImageUrls: String[]` alongside the legacy single `referenceImageUrl`, both models). Sidebar "Customised Order" button now opens `/store/custom-designs/new` directly (the form), not a list.
  - **[x] Then reworked again same day per follow-up feedback**: order-type/status/date filters and the "Placed by" dropdown were all **removed** (client wanted a simpler list, not more filtering) — replaced with a visible **Order Date** column and proper column headings (Order ID / Order Date / Status) for new users. A source-kind badge shows "Restock"/"Store Customer"/"Customised" per row (Kiosk is labelled "Store Customer", not "Kiosk", per client wording) — but the badge is hidden for a self-placed Restock order (redundant with "Placed by you" alone); Customised keeps its badge even when self-placed. A `placedByYou` boolean (derived from a null branch snapshot) drives an amber "Placed by you" badge, kept separate from the real branch-name badge so both can show together when relevant.
  - Per-item status badges were **restored** on `/store/b2b-orders` after being briefly missing — Retailer Admin (unlike the Store Manager) is meant to see the manufacturer's per-item status, matching what the manufacturer sees.
- **[x] Manufacturer's Catalogue Orders list (`/manufacturer/orders`)** also merges in Customised orders (additive — the separate `/manufacturer/custom-designs` page/nav entry stays untouched, per client's explicit "not touching that yet"). **Then the manufacturer's `/manufacturer/custom-designs` page itself was later emptied** (no data fetch/render, sidebar entry stays) per a same-day follow-up — its data now only surfaces via the merged list.
- **[x] Manufacturer order-view privacy — reverted partway through the same session.** First pass: stripped the retailer's business name/city/branch name from every manufacturer order view entirely (`getB2bOrdersByManufacturer` dropped the `store` join, `sanitizeKiosk`'s denylist gained `storeNameSnapshot`/`storeCitySnapshot`/`branchNameSnapshot`, `listCustomOrdersByManufacturer` dropped `storeNameSnapshot`, dashboard widget dropped `storeName`). **Client then asked for the business name back** (needs to identify who placed each order) while keeping city/branch hidden — so the business-name join/select was restored everywhere, `storeCitySnapshot`/`branchNameSnapshot` stayed stripped from `sanitizeKiosk`. See the Core Rules customer-PII entry above for the final state — this is the rule to trust, not the "removed entirely" phrasing that briefly existed mid-session.
- **[x] AI catalog-image prompt fixes** (`Jewel-Factory_AI` repo, `lib/prompts.py`) — several bangle-rendering issues found via client screenshots, fixed in sequence: (1) bangle looked like a flat 2D ribbon → added a rule that the band must show genuine 3D roundness/thickness regardless of camera angle; (2) bangle rendered as an open "C"-shaped arc with a gap → added a rule requiring a complete closed loop, telling the model to confidently extrapolate any hidden segment; (3) *(misdiagnosed once, then corrected)* bangle showed a fake clasp/hinge → root cause was actually a display prop/stand passing through the bangle's own circular opening, not invented hardware — fixed with a rule that props must support the bangle from OUTSIDE the closed loop only, and the camera-angle rule was softened from "near-flat" to a gentle three-quarter angle; (4) necklaces/earrings still generating too small despite the existing 55-70%-of-frame rule → added category-specific sizing guidance (crop the chain rather than shrink the pendant; a pair of earrings must hit the same dominant-footprint target as any other category).
- **[x] Registration form polish** — support/helpline number now shown near the submit button and in the always-visible portal footer (`PortalLoginScreen.tsx`, covers all 4 login/registration screens); Business Logo upload moved to the very last field (was mid-form); Business Email was already optional (confirmed, no change needed).
- **[x] Mobile-number login unified across both portals** — Retailer Admin and Retailer User already shared backend username=password=mobile-number logic; the Retailer User portal's login form was still frontend-labelled "Email address" with no mention of mobile login. Both portals now show "Mobile number" as the primary identifier label/placeholder in `PortalLoginScreen.tsx`.
- **[x] Custom Order form** — delivery date can no longer be set in the past (`min` = today's date, computed client-side); the Sample Weight field was removed entirely (client request, no longer collected).
- **[x] Responsive nav labels** — icon-only nav items (`StoreLayout.tsx` top nav, the Store Manager header's Cart button, `ItemsPerRowControl`) now show a visible text label at every screen width (a short abbreviated label below ~420px, full label above) instead of hiding the label below a breakpoint — new users could not tell what bare icons meant.
- **[x] Weight unit relabeled "g" → "gm"** everywhere it's displayed — `lib/format.ts`'s shared `formatWeight()` (used almost everywhere), plus several pages/components that built the "12.5g" string inline instead of through that helper (manufacturer/retailer intelligence pages, order item rows in `BranchOrderList`/`b2b-orders`/`pending-approvals`/manufacturer `orders`, the legacy public checkout, `WeightRangeSlider`, `AnalyticsFilterBar`, `ProductDetailModal`, the Custom Design forms' "Weight (gm)" labels).
- **[x] `docs/WHATSAPP_SETUP.md` added** — a complete one-sitting Meta WhatsApp Cloud API setup guide (Business Portfolio → App → WhatsApp product → real phone number → business profile/logo → business verification → permanent access token → two branded message templates for password-reset and account-approval), written so the guide's author only needs to log into Meta once. **This is prep work only — the actual WhatsApp send integration (forgot-password link via WhatsApp, approval notification via WhatsApp) is NOT implemented yet**, parked until the Meta setup is complete and credentials (access token, Phone Number ID, WABA ID, approved template names) are handed over. Forgot-password and approval-notification flows are UNCHANGED in this session — still fully email-based (the "recover email from mobile number" lookup flow at `/store/forgot-password` already existed and was not touched).
- **[x] Bug fix: `Hollow Bangles` sub-category renamed to `Hollow Pipe Bangles`** in `lib/categories.ts` (display-forward only, existing product rows keep their stored string).
- **Concurrently merged in from parallel commits on `master`** (Abhay Singh, unrelated to the above): `StoreManagerProductDetailModal` polish — scroll-snap fixes so a product never appears half-cropped mid-scroll, corrected `ItemsPerRowControl` +/- semantics (was inverted relative to plain arithmetic), and later a fixed-size-card modal layout with a sticky action footer + fading "Scroll to see similar" hint. No conflicts with this session's own centering/spacing edits to the same file — resolved via a real 3-way merge, not overwritten.
- **Not yet deployed** — the `20260804030000_remove_sales_person_multi_image` migration (this session) needs `prisma migrate deploy` + container rebuild on EC2. No other schema changes this session.

**Previous session (2026-08-03/04) — order-status rework, per-item status, JFA-/JFC- order numbering, misc client punch-list items:**
- **[x] Order-status enum replaced** (migration `20260803020000_order_status_rework`) on `OrderStatus` (Kiosk + Catalog/B2B) and `CustomOrderStatus` (Customised) — old `PENDING/CONFIRMED/PACKED/SHIPPED/DELIVERED/CANCELLED` (+`IN_PRODUCTION` on custom) replaced with `PENDING/IN_PROCESS/GHAT_RECEIVED/READY_FOR_DELIVERY/DISPATCHED/COMPLETED/CANCELLED`, matching the client's reference screenshot. Historical rows are remapped (`CONFIRMED→IN_PROCESS`, `PACKED→GHAT_RECEIVED`, `SHIPPED→DISPATCHED`, `DELIVERED→COMPLETED`) by the migration itself (Postgres enum recreate + `USING CASE` cast — enum values can't be dropped in place). **The B2B "on delivery, materialize into store inventory" trigger and the 6 analytics queries that keyed off `'DELIVERED'` now key off `'COMPLETED'`** (`lib/db/orders.ts`, `lib/db/analytics-queries.ts`) — don't reintroduce a `DELIVERED` check anywhere. New shared `formatOrderStatus()` in `lib/format.ts` turns `READY_FOR_DELIVERY` → "ready for delivery" (multi-underscore safe) — use it instead of ad-hoc `.replace('_',' ').toLowerCase()`, which silently breaks on more than one underscore.
- **[x] Per-line-item status** (migration `20260803030000_order_item_status`) — `KioskOrderItem`/`B2bOrderItem` each gained their own `status OrderStatus` (default PENDING), independent of the order-level status, since one order can have products at different production stages. **Custom design orders excluded** (generally a single design per order). Manufacturer advances each item via new `PATCH /api/manufacturer/{orders,kiosk-orders}/:id/items/:itemId` (`advanceB2bOrderItemStatus`/`advanceKioskOrderItemStatus` in `lib/db/orders.ts`); Retailer Admin (`/store/b2b-orders`) and Retailer User (`/store-manager/my-orders`) both see the same per-item badge read-only.
- **[x] JFA-/JFC- order numbering** (migration `20260804000000_manufacturer_order_seq`, new `lib/db/order-number.ts`) — `Manufacturer` gained `nextCatalogOrderSeq`/`nextCustomOrderSeq`, one running counter **per manufacturer**, shared across every retailer (and their Retailer Users) that manufacturer serves. Kiosk **and** Catalog/B2B orders draw from the same `nextCatalogOrderSeq` counter → `JFA-####` (they're already merged into one "Catalogue Orders" list everywhere, so one shared numbering matches that). Customised orders get their own `nextCustomOrderSeq` → `JFC-####`. Old formats (`GK-YYYYMMDD-XXXX`, `B2B-YYYYMMDD-XXXX`, `CD-YYYYMMDD-XXXX`) are gone for NEW orders; existing rows keep their old `orderNumber` untouched (no backfill). The counter increments via an atomic `UPDATE ... RETURNING` (not read-then-write), so concurrent placements never collide — see `lib/db/order-number.ts` for why a raw query was used instead of a `$transaction`.
- **[x] Sales Code + Sales Person Name** (migration `20260803010000_kiosk_sales_person`) — captured as required fields at the Store Manager's Kiosk "Add to Cart" checkout (`kiosk_orders.sales_code`/`sales_person_name`), shown to the Retailer Admin on Catalogue Orders. **Restock is unaffected** (no sales-person concept there).
- **[x] "Requirement note" → "Remark"** display rename (kiosk/B2B cart panel, Pending Approvals, manufacturer order detail, About page copy) — same display-text-only precedent as earlier renames; `requirementNote` DB/API field name unchanged.
- **[x] Bangles sub-categories reordered** (`lib/categories.ts`) — Top Seller → Premium → **Ultra Light Bangles** (new) → Fancy Hmade → Fusion → (rest unchanged) → Raji Bangles (renamed from Reli) → **Nakshi Bangles** (new, added at the very end per client confirmation).
- **[x] Retailer Admin can place a Customised Order directly** — new `POST /api/store/custom-designs` (auto-forwards immediately, same self-approval-bypass precedent as the B2B fix below) + `/store/custom-designs/new` form + a "Place Customised Order" button on `/store/custom-designs`.
- **[x] Customised Order requests now also show on `/store/pending-approvals`** with Approve/Reject, alongside Kiosk/Catalog orders (previously only reachable via the separate Customised Orders list page).
- **[x] "Product Catalogue" removed from the Retailer Admin top navbar** (`components/layout/StoreLayout.tsx` `TOP_NAV`) — Manufacturer Catalogue browsing still reachable via Dashboard/other links.
- **Custom Design form fields made required** (`app/store-manager/custom-design/page.tsx`) — all previously-optional counter-spec fields are now required; `Meena` options narrowed to `Yes`/`No`; `Screw` options narrowed to `English`/`Pongli`; `Size` placeholder guides free-text entry (`"2.6"` or `"2.6.5"`); weight inputs format to 3 decimals on blur; `Quantity` is now free text (`"2 pcs"`) — schema column widened `Int?`→`String?` (migration `20260803000000_custom_design_quantity_text`).
- **[x] "User Mobile Number"** replaces "Retail User Number" label on the Retailer Admin's Add Store/Retailer User form (`/store/branches`).
- **Not yet deployed** — 6 new migrations this session (see Migrations section) need `prisma migrate deploy` + container rebuild on EC2.

**Previous session (2026-08-01) — cart UX, "Catalogue" rename, customised-order spec fields:**
- **[x] Product popup no longer closes on add** anywhere (`/store/manufacturer-catalog`, `/store/similar-search`, `/store-manager/{kiosk,restock,search}`). Similar designs are listed below the opened one, so closing on add defeated the point.
- **[x] New `components/orders/CartQtyControl.tsx`** — one control that is a gold "Add" button at qty 0 and a `− n +` stepper above it; decrementing from 1 removes the line. Replaces the dead "In cart" badge, which had no way back out of the cart. Used on the retailer catalogue cards, the Store Manager kiosk/restock cards (now **always visible**, not hover-only — these run on touch screens) and inside the product popup on all 5 caller pages.
- **[x] "Qty" column header** above the steppers in both cart panels.
- **[x] "Catalog" → "Catalogue"** across all display text (23 files, incl. nav labels, page headings, landing/about copy, manufacturer AI-generate labels). **Routes, API paths, component names (`CatalogOrderPanel`, `CatalogBrowse`, `CatalogFilters`), `catalogHref` and DB identifiers are unchanged** — same display-text-only precedent as the earlier renames.
- **[x] Customised-order spec fields** (migration `20260801000000_custom_design_spec_fields`) — `sub_category`, `order_ref`, `delivery_date`, `quantity`, `meena`, `length`, `size`, `broadness`, `screw`, `sample_weight_grams` added to **both** `custom_design_requests` and `custom_design_orders`, all nullable. `forwardCustomRequest` copies the whole spec through to the manufacturer (it carries no PII). New shared `components/orders/CustomSpecList.tsx` renders only the filled fields, used by the Retailer User (My Orders), Retailer Admin (Customised Orders) and manufacturer.
  - `orderRef` is the **shop's own** order number — NOT `CustomDesignOrder.orderNumber` (the system's `JFC-####`, see the 2026-08-04 entry below). Don't merge them.
  - Sub-category now has its own column; it used to be glued onto `category` as `"Bangles — Fusion Bangle"`. Old rows keep the glued form, so readers print `category` as-is and append `subCategory` when present.
  - `MEENA_OPTIONS` / `SCREW_OPTIONS` in `app/store-manager/custom-design/page.tsx` are **guessed shop vocabulary** — plain constants, not validated server-side (the columns are free text). Confirm the real option lists with the client and edit those two arrays.
  - The form's "Requirement / notes" is now labelled **Remarks** and still writes `designNotes`.
- **[x] Bangle size on the manufacturer catalogue** (migration `20260802000000_product_size`) — optional free-text `size` on `ManufacturerProduct`, and the Add/Edit Design form only renders the field when **category === 'Bangles'**. Switching away from Bangles sends `size: null`, so a stale size can't linger on a non-bangle design. Shown in the shared product detail modal (retailer + store manager) only when set. The column itself is unconstrained — the Bangles-only rule lives in the form, not the DB.
- **[x] Add/Edit Design form reordered** — Size sits on its own row directly under Weight (Bangles only), and the whole "Generate with AI" block moved **below Karigar Code, above Catalogue Photos**, so the specs are filled before the AI step that consumes them.
- **[x] "Draft" is now displayed as "Inactive"** in the manufacturer portal (status dropdown + catalogue badge via `STATUS_LABELS`). **The `ProductStatus.DRAFT` enum value is unchanged** — display-text-only, same precedent as the other renames.
- **[x] Catalogue status filter + bulk activate** (`/manufacturer/catalog`) — new All/Active/Inactive/Archived dropdown. Picking **Inactive** turns the grid into selection mode: cards select instead of navigating, with "Select all" and a "Make active (n)" button. New endpoint `POST /api/manufacturer/products/bulk-status` (`setManufacturerProductsStatus`, an `updateMany` scoped by `manufacturerId`) does it in one request instead of N PATCHes. Selection is deliberately gated to the Inactive filter — that's the only view where one action applies to everything in it.
- **Not yet deployed** — `prisma migrate deploy` + container rebuild needed.

**Previous session (2026-07-31) — email-optional purchase manager registration + auth login responsiveness:**
