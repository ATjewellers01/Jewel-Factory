...
type RawCategory = { id: string; name: string; subCategories1: { id: string; name: string }[]; subCategories2: { id: string; name: string }[] };
...
export type Taxonomy = {
  ...
  subCategories2For: (category: string | null | undefined) => string[];
  loaded: boolean;
};
...
export function useTaxonomy(endpoint: string): Taxonomy {
  ...
  return {
    ...
    subCategories2For: (category) => raw.categories.find((c) => c.name === category)?.subCategories2.map((s) => s.name) ?? [],
    loaded: true,
  };
}
